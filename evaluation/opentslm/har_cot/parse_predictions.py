...
def parse_rtf_jsonl(input_file, output_dir=None):
    ...
    if extracted_data:
        summary_lines = [f"Input file: {input_file}", f"Output file: {output_file}"]
        ...
        if excluded_count > 0:
            ...
            summary_lines.append(f"Excluded {excluded_count} predictions not in the label set from metrics")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Parse HAR-CoT predictions from JSONL file")
    ...
    # Parse the file
    parse_rtf_jsonl(args.file_path, args.output_dir)
